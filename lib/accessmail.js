...
var checkTraining = function(){
	...
	training.forEach(function(course){
    	...
    	course.accessmail.days = course.accessmail.days || [1];
    	course.accessmail.hour = course.accessmail.hour || 12;
    	if(
        	course.accessmail.sendon === "weekly" && 
        	(
            	course.accessmail.days.indexOf(now.getDay()) === -1 ||
             	course.accessmail.hour !== now.getHours()
            ) 
        ) return;
    	logs.findOne({action: "accessmail."+course.label}, {sort: {time: -1}}, function(err, log){
        	if(err) return console.error(err);
        	if(!log) log = {action: 'accessmail.'+course.label, details: {}, time: 0};
        	log.time = new Date(log.time);
        	if(course.accessmail.sendon === 'weekly' && Date.now() - log.time.getTime() < 24*60*60*1000) return;
        	perms.find({perm: {$in: course.perms}, added: {$gt: log.time}, enabled: true}).toArray(function(err, ps){
            	...
            	users.find({username: {$in: usernames}}).toArray(function(err, doorusers){
                	...
                	var email = Object.assign({}, course.accessmail.email);
                	var template;
                	if(email.text){
                	...
                	sendMail(email);
          			logs.insertOne({action: "accessmail."+course.label, details: {users: doorusers, training: course.label}, time: new Date()});
                });
            });
        });
    });
